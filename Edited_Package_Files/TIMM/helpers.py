### This script presents our modifications to the TIMM.Models.helpers script
...
def load_checkpoint(model, checkpoint_path, use_ema=False, strict=True):
    ...
    if new_SD:
        #### remapp here
        map = {'classifier.0.weight': u'pre_logits.fc1.weight', 'classifier.0.bias': u'pre_logits.fc1.bias',
               'classifier.3.weight': u'pre_logits.fc2.weight', 'classifier.3.bias': u'pre_logits.fc2.bias',
               'classifier.6.weight': u'head.fc.weight', 'classifier.6.bias': u'head.fc.bias'}

        sd = OrderedDict([(map[k] if k in map else k, v) for k, v in state_dict.items()])
        state_dict = sd

        # #### reshape
        state_dict['pre_logits.fc1.weight'] = state_dict['pre_logits.fc1.weight'].reshape(4096, 512, 7, 7)
        state_dict['pre_logits.fc2.weight'] = state_dict['pre_logits.fc2.weight'].reshape(4096, 4096, 1, 1)

        #### remove entries
        # list = ["AuxLogits.conv0.conv.weight", "AuxLogits.conv0.bn.weight", "AuxLogits.conv0.bn.bias", "AuxLogits.conv0.bn.running_mean", "AuxLogits.conv0.bn.running_var", "AuxLogits.conv1.conv.weight", "AuxLogits.conv1.bn.weight", "AuxLogits.conv1.bn.bias", "AuxLogits.conv1.bn.running_mean", "AuxLogits.conv1.bn.running_var", "AuxLogits.fc.weight", "AuxLogits.fc.bias"]
        #
        # for l in list:
        #     del state_dict[l]
...
def load_pretrained(model, default_cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True, progress=False,
    ...
    if hf_hub_id and has_hf_hub(necessary=not pretrained_url):
        ...
        state_dict = load_state_dict_from_url(pretrained_url, file_name=file_name, progress=progress,
                                              map_location='cpu')
...
def adapt_model_from_string(parent_module, model_string):
    ...
    for n, m in parent_module.named_modules():
        ...
        if isinstance(old_module, nn.Linear):
            #  extra checks to ensure this is actually the FC classifier layer and not a diff Linear layer?
...
def update_default_cfg_and_kwargs(default_cfg, kwargs, kwargs_filter):
    """ Update the default_cfg and kwargs before passing to model
     ...
     this sequence of overlay default_cfg, set default kwargs, filter kwargs
...
def build_model_with_cfg(
    ...
    if pretrained:
        if pretrained_custom_load:
            load_custom_pretrained(model, checkpoint_path=file_name)
        else:
            load_pretrained(
                model,
                file_name=file_name,
...
def model_parameters(model, exclude_head=False):
    if exclude_head:
        #  this a bit of a quick and dirty hack to skip classifier head params based on ordering
