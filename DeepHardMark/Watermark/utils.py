...
def compute_loss_statistic(model, images, target, epsilon, G, args, imgnet_normalized_ops, B):
	loss = compute_loss(model, images, target, epsilon, G, args, imgnet_normalized_ops, B)
	statistics = compute_statistics(images, epsilon, G, args, B)
	
	results = {'loss': loss, 'statistics': statistics}
	return results


def compute_predictions_labels(model, images):
	...
	predictions = model(images)  # 1*c variable, c is the class
	...
	if type(images) == torch.tensor or type(images) == torch.Tensor:
		return predictions_labels.detach(), images.detach()  # .detach() is similar to .data in pytorch 0.3.1, that is to return a tensor
	else:
		images["input_ids"] = images["input_ids"].detach()
		images["attention_mask"] = images["attention_mask"].detach()
		return predictions_labels.detach(), images
